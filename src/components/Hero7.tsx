        ...
        >
          ...
          <div>
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, index) => (
                <Icon key={index} name="BookOpen" className="size-5 text-indigo-500" />
              ))}
              <span className="mr-1 font-semibold">{reviews.rating?.toFixed(1)}</span>
            </div>
            <p className="text-muted-foreground text-left font-medium">{reviews.count}+ научных публикаций</p>
          </div>
        </motion.div>
      </div>
    </section>
